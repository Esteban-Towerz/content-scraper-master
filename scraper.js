...
const request  = require('request');  // Node module for making HTTP request
	...
	if(!error && response.statusCode === 200) {
		...
		for (let i = 0; i < shirtArray.length; i++) {
			...
			if (shirtArray[i].indexOf("?id=") > 0) {
				...
				request(shirtArray[i], (error, response, body) => {
					...
					if(!error && response.statusCode === 200) {
						...
						for (var i = 0; i < shirtToScrap.length; i++) {
							...
							request(shirtToScrap[i], (error, response, body) => {
								...
								if (!error && response.statusCode == 200) {
									...
									json2csv({data:data, fields:csvFields}, (err, csv) => {

										if (err) throw err;
										/** If the data file for today already exists it should overwrite the file */
										fs.writeFile(dir + "/" + csvFileName, csv, (err) => {
											if (err) throw err;
...
function printErrorMessage(error) {
	...
	fs.appendFile(errorFile, errorMsg, (err) => {
		if (err) throw err;
